...
describe('buildCvModel', () => {
  test('monta o CV em EN com dados do site/experiência/skills', () => {
    ...
    expect(cv.experience[0].roles[0].role).toBe('Senior Software Developer')
    expect(cv.experience[0].roles[1].role).toBe('Mid Software Developer')
    expect(cv.experience[0].roles[0].bullets.length).toBeGreaterThan(0)
  ...
  test('localiza para PT', () => {
    ...
    expect(cv.experience[0].roles[0].role).toBe('Desenvolvedor de Software Sênior')
  })
})
